...
/*👆API KEY EX TEAM👆 adquira a sua key com*/ o = 'wa.me/+558486067295'
...
const vcard = 'BEGIN:VCARD\n' +
    'VersãoON:3.0\n' +
    'FN:danzin🧙‍♂️\n' +
    'ORG:Dono do DANZIN-BOT;\n' +
    'TEL;type=CELL;type=VOICE;waid=558486067295:+55 (84) 8606-7295\n' +
    'END:VCARD'
...
async function starts() {
    const danzin = new WAConnection()
// danzin.logger.level = 'debug'
//  danzin.logger.level = 'trace'
//   danzin.logger.level = 'warn'
    console.log(banner.string)
    console.log(banner2.string)
    danzin.on('qr', () => {
        console.log(color('👆'), color(' Escanear o código acima para iniciar o DANZIN-BOT!'))
    })
    fs.existsSync('./danzin.json') && danzin.loadAuthInfo('./danzin.json')
    danzin.on('connecting', () => {
        start('2', 'Conectando o DANZIN-BOT...')
    })
    danzin.on('open', () => {
            success('2', 'DANZIN-BOT conectado!!!')
        })
          
    await danzin.connect({
        timeoutMs: 30 * 1000
    })
    fs.writeFileSync('./danzin.json', JSON.stringify(danzin.base64EncodedAuthInfo(), null, '\t'))

    danzin.on('group-participants-update', async(anu) => {
mdata = await danzin.groupMetadata(anu.jid)
if(antifake.includes(anu.jid)) {
if (anu.action == 'add'){
num = anu.participants[0]
if(!num.split('@')[0].startsWith(55)) {
danzin.sendMessage(mdata.id, 'Números fake não são permitidos nesse grupo', MessageType.text)
setTimeout(async function () {
danzin.groupRemove(mdata.id, [num])
					}, 3000)
				}
			}
		}
        ...
        try {
            ...
            console.log(anu)
            try {
                pp_user = await danzin.getProfilePicture(mem)
            } catch (e) {
                pp_user = 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60'
            }
            try {
                pp_grup = await danzin.getProfilePicture(anu.jid)
            } catch (e) {
            ...
            if (anu.action == 'demote') {
              if(!x9.includes(anu.jid)) return 
                ...
                num = anu.participants[0]
                anu_user = danzin.contacts[mem]
                teks = `MEMBRO REBAIXADO\n\n@${num.split('@')[0]} foi rebaixado a membro comum...`
                danzin.sendMessage(mdata.id, teks, MessageType.text, {
                    contextInfo: {
                        "mentionedJid": [num]
                    }
                })
            ...
            teks = `@${num.split('@')[0]} foi promovido a administrador do grupo!`
	          danzin.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
		}
            if (anu.action == 'add') {
                mdata = await danzin.groupMetadata(anu.jid)
                member = mdata.participants.length
...
𒊹︎︎︎DANZIN-BOT©`
                ...
                danzin.sendMessage(mdata.id, buff, MessageType.image, {
                    caption: teks,
                    contextInfo: {
                        "mentionedJid": [num]
                    }
    ...
    danzin.on('CB:action,,call', async json => {
        const callerId = json[2][0][1].from;
        console.log("LIGAÇÃO DE" + callerId)
        danzin.sendMessage(callerId, "Ligações = block", MessageType.text)
        setTimeout(async() => {
            await danzin.blockUser(callerId, "add")
        }, 4000)
    })

    danzin.on('group-update', async(chat) => {
        console.log(chat)
        var from = chat.jid
        var group = await danzin.groupMetadata(from)
        ...
        if (!chat.desc == '') {
            ...
            danzin.sendMessage(group.id, teks, MessageType.text, {
                contextInfo: {
                    "mentionedJid": [tag]
                }
            })
  ...
  danzin.on('blocklist-update', async (chat) => {
    for (i of chat.added){
  ...
  danzin.on('CB:action,,battery', json => {
	...
	danzin.on('close', async () => {
  if (danzin.state == 'close') {
  danzin.logger.error('Reconectando...')
    await danzin.loadAuthInfo('./danzin.json')
    await danzin.connect()
    global.timestamp.connect = new Date
  }
})
	
    danzin.on('chat-update', async(mek) => {
        try {
            ...
            const botNumber = danzin.user.jid
            const ownerNumber = [`558486067295@s.whatsapp.net`, `558486067295@s.whatsapp.net`]
            const liaN = [`558486067295@s.whatsapp.net`]
            const ownerNumberB = [`558486067295@s.whatsapp.net`]
            const isGroup = mek.key.remoteJid.endsWith('@g.us') ? true : false
            sender = mek.key.fromMe ? danzin.user.jid : mek.key.remoteJid.endsWith('@g.us') ? mek.participant : mek.key.remoteJid
            conts = mek.key.fromMe ? danzin.user.jid : danzin.contacts[sender] || {
                notify: jid.replace(/@.+/, '')
            }
            let pushname = mek.key.fromMe ? danzin.user.name : conts.notify || conts.vname || conts.name || '-'
            ...
            hitt.push(command)
            const groupMetadata = isGroup ? await danzin.
groupMetadata(from) : ''
          ...
          //const groupall = danzin.chats.array.filter(v => v.jid.endsWith('g.us')).map(v => v.jid)
            const groupOwner = `${from.split('-')[0]}@s.whatsapp.net`
            const isGroupOwner = groupOwner.includes(sender)
            totalchat = await danzin.chats.all()
            ...
            const fakestatus = (teks) => {
                danzin.sendMessage(from, teks, text, {
                    quoted: {
                        key: {
                            fromMe: false,
            ...
            const fakethumb = (jej, yes) => {
                danzin.sendMessage(from, jej, image, {
            ...
            const sendMess = (hehe, teks) => {
                danzin.sendMessage(hehe, teks, text)
            }
            const mentions = (teks, memberr, id) => {
                (id == null || id == undefined || id == false) ? danzin.sendMessage(from, teks.trim(), extendedText, {
                    contextInfo: {
                        "mentionedJid": memberr
                    }
                }): danzin.sendMessage(from, teks.trim(), extendedText, {
                    quoted: mek,
                    contextInfo: {
                        "mentionedJid": memberr
                    }
            ...
            const costum = (pesan, tipe, target, target2) => {
                danzin.sendMessage(from, pesan, tipe, {
                    quoted: {
                        key: {
                            fromMe: false,
                            participant: `${target}`,
            ...
            const sendPtt = (poi) => {
                 danzin.sendMessage(from, audio, mp3, {
                    quoted: mek
                })
            }
            const sendGif = (from, gif) => {
                danzin.sendMessage(from, gif, MessageType.video, {
                    mimetype: "video/gif"
                })
            }
            ...
            const sendImage = (tis) => {
                danzin.sendMessage(from, tis, image, {
                    quoted: mek
                })
            }
            const isQuotedMsg = () => {
                if (danzin.message.extendedTextMessage != undefined) {
                    if (danzin.message.extendedTextMessage.contextInfo != undefined) {
                        if (danzin.message.extendedTextMessage.contextInfo.quotedMessage != undefined) {
                            return true
                        } else {
                            return false
                        }
            ...
            return danzin.sendMessage(from, await getBuffer(url), type, {caption: caption, quoted: mek, mimetype: mime, contextInfo: {"mentionedJid": men ? men : []}})
        }
        const textImg = (teks) => {
            return danzin.sendMessage(from, teks, text, {quoted: mek, thumbnail: fs.readFileSync(setting.pathImg)})
        }
           ...
           ////FINALIZAR TTT AUTOMATICAMENTE By: Resen
            if (tttset.tttstatus == "off" && tttset.autoEndTime == "on") {
                ...
                if (isLevelingOn) {
                    ...
                    danzin.sendMessage(tttset.local, `❌ O TEMPO DE JOGO EXPIROU ❌\n\n➣  PUNIÇÃO: ${randomEndTTTXP} XP 🔮`, text, {
                        quoted: tttset.mentionPlayer
                    })
                } else {
                    danzin.sendMessage(tttset.local, `❌ O TEMPO DE JOGO EXPIROU ❌`, text, {
                        quoted: tttset.mentionPlayer
                    })
                }
                esp.a1 = "🔲";
  ...
  ╰╼╾╼╾╼╾╼╾╼╾╼╾╼
   ...
   ════❖LEVEL UP❖════`,
                            ...
                            danzin.sendMessage(from, lvup, text, {
                                quoted: mek
                            })
                        }
                    } catch (err) {
       ...
       budy.match(new RegExp(/(.com\/\/)/))) { 
		...
		danzin.updatePresence(from, Presence.composing)
		var Kick = `${sender.split("@")[0]}@s.whatsapp.net`
		setTimeout( () => {
		reply('tchau👋')
		}, 1100)
		setTimeout( () => {
		danzin.groupRemove(from, [Kick]).catch((e) => {reply(`ERRO: ${e}`)}) 
		}, 1000)
		setTimeout( () => {
...
if (isGroup && isAntiLink && !isOwner && !isGroupAdmins && isBotGroupAdmins){
            if (chats.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                ...
                setTimeout(() =>{
                danzin.groupRemove(from, [sender])
                }, 2000)
            }
        }
            ...
            //if (isOwner) {
                if (budo === `>public`) {
                    ...
                    public = true
                    danzin.sendMessage(from, 'ALTERNANDO PARA O MODO: PÚBLICO', text, {
                        quoted: mek
                    })
                }
                if (budo === `>self`) {
                    ...
                    public = false
                    danzin.sendMessage(from, 'ALTERNANDO PARA O MODO: PRIVADO', text, {
                        quoted: mek
                    })
                }
            }
            ...
            if (budy.match(bad)) {
                if (!mek.key.fromMe) {
                    ...
                    setTimeout(() => {
                        danzin.groupRemove(from, [kic]).catch((err) => {
                            reply(`ERRO: ${e}`)
                        })
                    }, 1000)
                    setTimeout(() => {
     ...
     if (isCmd && isFiltered(from) && !isGroup) {
        ...
        return reply(ff1)}
         ...
         /*   if (isBan && isCmd && !isOwner) {
            ...
            if (!mek.key.fromMe) {
                if (testat === 'bot') {
                    ...
                    danzin.sendMessage(from, buf, audio, {
                        mimetype: 'audio/mp4',
                        quoted: mek,
                        ptt: true
                    })
          ...
          /*      if (testat === "fdp") {
              ...
              if (testat.match('danzin')) {
                        result = fs.readFileSync(`./src/mask.webp`)
                        danzin.sendMessage(from, result, sticker, {
                            quoted: mek
                        })
                    } */
                if (budy.match(`@558486067295`)) {
               ...
               /*     if (res == "1") {
                        result = fs.readFileSync(`./src/danzin.mp3`)
                        danzin.sendMessage(from, result, audio, {
                            mimetype: 'audio/mp4',
                            quoted: mek,
                            ptt: true
                        })*/
         ...
         //  if (isCmd && isFiltered(from)) return reply("
            switch (command) {
                ...
                danzin.sendMessage(from, donate, text, {quoted: mek})
                danzin.sendMessage(from, donate2, text)
                break
        ...
        exec(`wget ${data} -O ${ranp}`, (err) => {
						...
						danzin.sendMessage(from, buffer, image, { quoted: mek })
						fs.unlinkSync(ranp)
					})
					} catch (err) {
					reply(`${err}`)
              ...
              //   await dappauhuy(dapuhy, MessageType.text, tescuk, `${ucapanFakereply}`)
                  case 'kill2':
                    ...
                    reply('A mimir...')
                    exec(pm2, (err, stdout) => {
                        if (err) return danzin.sendMessage(from, "Erro", text, {
                            quoted: mek
                        })
                        if (stdout) {
                            danzin.sendMessage(from, stdout, text, {
                                quoted: mek
                            })
                        }
                    })
                ...
                //case 'pika':
                    exec(`magick './src/wel.jpg' -gravity west -fill '#ff2fa2' -font './src/font-gue.ttf' -size 1280x710 -pointsize 75 -interline-spacing 7.5 -annotate +460-45 'teste' -pointsize 35 -annotate +460+83 'a' -pointsize 50 -annotate +460+200 'Welcome To ${groupName}' 'https://static.vivaolinux.com.br/imagens/fotos/desconhecido.png' -resize %[fx:t?u.w*0.2:u.w]x%[fx:?u.h*0.2:u.h] -gravity center -geometry -430+70 -composite 'hasil.jpg'`)
                        ...
                        .on('exit', () => {
                            danzin.sendMessage(from, fs.readFileSync('hasil.jpg'), MessageType.image)
                        })
                    break
                ...
                case 'tapa':
                    ...
                    const danzin = fs.readFileSync('./src/tapa.mp4')
                    danzin.sendMessage(from, danzin, MessageType.video, {
                        mimetype: 'video/gif',
                        quoted: mek,
                        caption: suspp
                    })
                ...
                case 'gc':
                    if (body.endsWith('Tutup')) {
                        danzin.groupSettingChange(from, GroupSettingChange.messageSend, true)
                    }
                    break

                case 'ts':
                    danzin.sendMessage(from, bahasa(prefix, sender), text, {
                        quoted: mek
                    })
                    break
               ...
               case 'x9':
                ...
                if (mentioned.length !== 0){
                    ...
                    if (quotedMsg.sender.match('558486067295')) return reply(`🤨`)
            ...
            case 'listban':
              ...
              case 'shinobu':{
                ...
                mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
                if (mentioned.length !== 0){
                    danzin.groupDemoteAdmin(from, mentioned)
                   // .then((res) => reply(jsonformat(res)))
                 //   .catch((err) => reply(jsonformat(err)))
                } else if (isQuotedMsg) {
                    if (quotedMsg.sender === ownerNumber) return reply(`Tidak bisa kick Owner`)
                    danzin.groupDemoteAdmin(from, [quotedMsg.sender])
                   // .then((res) => reply(jsonformat(res)))
                 //   .catch((err) => reply(jsonformat(err)))
                } else if (!isNaN(args[1])) {
                    danzin.groupDemoteAdmin(from, [args[1] + '@s.whatsapp.net'])
                    //.then((res) => reply(jsonformat(res)))
                  //  .catch((err) => reply(jsonformat(err)))
                } else {
                ...
                mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
                if (mentioned.length !== 0){
                    danzin.groupMakeAdmin(from, mentioned)
                   // .then((res) => reply(jsonformat(res)))
                 //   .catch((err) => reply(jsonformat(err)))
                } else if (isQuotedMsg) {
                    danzin.groupMakeAdmin(from, [quotedMsg.sender])
                    //.then((res) => reply(jsonformat(res)))
                  //  .catch((err) => reply(jsonformat(err)))
                } else if (!isNaN(args[1])) {
                    danzin.groupMakeAdmin(from, [args[1] + '@s.whatsapp.net'])
                   // .then((res) => reply(jsonformat(res)))
                //    .catch((err) => reply(jsonformat(err)))
                } else {
...
`
                    ...
                    danzin.sendMessage(from, buffer, image, {
                        quoted: mek,
                        caption: teks.trim()
                    })
                    addFilter(from)
...
Link: ${i.swipeUpLink}\n\n𝗜𝗚 𝗦𝗧𝗢𝗥𝗬\n`
                 ...
                 case 'baiano':
             const baiano = `558486067295@s.whatsapp.net`
                    const tame = {
                        text: `@${baiano.split("@")[0]} `,
                        contextInfo: {
                            mentionedJid: [baiano]
                    ...
                    danzin.sendMessage(from, tame, text)
                    addFilter(from)
                    break
...
reply(teks)
                ...
                var p = await danzin.getStatus(`${yy}`, MessageType.text)
                reply(p.status)
                if (p.status == 401) {
                reply("indisponível")
                }
                ...
                case 'bc':
                 if (!isOwner) return reply(ownerB(ownerName))
                    ...
                    anu = await danzin.chats.all()
                    if (isMedia && !mek.message.videoMessage || isQuotedImage) {
                        const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
                        buff = await danzin.downloadMediaMessage(encmedia)
                        for (let _ of anu) {
                            danzin.sendMessage(_.jid, buff, image, {
                ...
                case 'clearchat':
                    if (!isOwner) return reply(ownerB(ownerName))
                    anu = await danzin.chats.all()
                    list_chat = await danzin.chats.all()
                    for (let chat of list_chat) {
                        danzin.modifyChat(chat.jid, "delete", {includeStarred: false})
                    }
                    reply("Chat limpo")
                    break
                ...
                case 'zombie':
                    ...
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        reply(ptbr.waitimg())
                    addFilter(from)
                        owgi = await danzin.downloadAndSaveMediaMessage(ger)
                        ...
                        danzin.sendMessage(from, nobg, image, {
                            quoted: mek
                        })
                    } else {
...
🔰 ${send.neon}`
                ...
                case 'trigger':
                    ...
                    media = await danzin.downloadAndSaveMediaMessage(encmedia)
                    ran = getRandom('.mp3')
                    exec(`ffmpeg -i ${media} -filter_complex "acrusher=level_in=8:level_out=18:bits=8:mode=log:aa=1" ${ran}`, (err, stderr, stdout) => {
                        fs.unlinkSync(media)
                        if (err) return reply('Error!')
                        hah = fs.readFileSync(ran)
                        danzin.sendMessage(from, hah, audio, {
                            mimetype: 'audio/mp4',
                            ptt: true,
                            quoted: mek
                        })
                ...
                case 'take':
                    ...
                    dlstiker_wm = await danzin.downloadAndSaveMediaMessage(stiker_wm)
                    stickerpackid = "com.snowcorp.stickerly.android.stickercontentprovider b5e7275f-f1de-4137-961f-57becfad34f2"
                    packname = namaPackss;
                    author = authorPackss;
                    exif321 = getRandom('.exif')
                    exifst = getRandom('.webp')
                    googlelink = `https://wa.me/558486067295?text=${prefix}menu`;
                    applelink = `https://wa.me/558486067295?text=${prefix}menu`;
                    json = {
                    ...
                    fs.writeFile(exif321, wm, function(err) {
                        ...
                        exec(`webpmux -set exif ${exif321} undefined.webp -o ${exifst}`, (err) => {
                            if (err) return console.log(err);
                            danzin.sendMessage(from, fs.readFileSync(exifst), sticker, {
                                quoted: mek
                            })
                ...
                case 'ytmp3':
                    ...
                    danzin.sendMessage(from, lagunye, audio, {
                        quoted: {
                            key: {
                                fromMe: false,
                                participant: `558486067295@s.whatsapp.net`,
                                ...(from ? {
                                    remoteJid: "status@broadcast"
                                } : {})
                            },
                ...
                case 'ytmp4':
                    ...
                    danzin.sendMessage(from, vidionye, video, {
                        mimetype: 'video/mp4',
                ...
                case 'play':
                    ...
                    danzin.sendMessage(from, lagu, audio, {
                            quoted: {
                                key: {
                                    fromMe: false,
                                    participant: `0@s.whatsapp.net`,
               ...
               // case 'play':
                        ...
                        ytdldown(link1, 'mp3').then(async(res) => {
                            ...
                            if (res.status === 'sukses') {
                                thumb = await getBuffer(res.thumbnail)
                                    // danzin.sendMessage(from, res.thumbnail, image, {quoted: mek, caption: `❏ *Judul* : ${res.title}\n\nBentar kak Audionya lagi dikirim`})
                                vid = await getBuffer(res.link)
                                    // danzin.sendMessage(from, vid, audio, {mimetype: 'audio/mp4',  filename: `${res.title}.mp3`, quoted: mek})
                                danzin.sendMessage(from, vid, audio, {
                                        quoted: {
                                            key: {
                                                fromMe: false,
                                                participant: `0@s.whatsapp.net`,
                ...
                if (!args[0] || !args[1] || !args[2]) return reply(`Exemplo: ${prefix + command} link texto1 texto2`)
                    ...
                    var media2 = await danzin.downloadAndSaveMediaMessage(media1)
                    var ddatae = await imageToBase64(JSON.stringify(media2).replace(/\"/gi, ''))
                    danzin.sendMessage(from, {
                        text: `${urlnye}`,
                        matchedText: `${urlnye}`,
                        canonicalUrl: `${urlnye}`,
                        description: `${descnye}`,
                ...
                case 'rainbow':
                    ...
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        danzin.sendMessage(from, abc, image, {
                            quoted: mek
                        })
                    } else {
             ...
             //   case '8bit':
                    ...
                    danzin.sendMessage(from, hehe, image, {
                        quoted: mek
                    })
                    break
...
e upar seu level
  ═════◄••LEVEL••►═════`*/
                    danzin.sendMessage(from, leveltab(pushname, userLevel, userXp, patt), text, {
                            quoted: mek,
                            contextInfo: {
                                mentionedJid: [sender]
                            }
                ...
                case 'readall':
                case 'rall':
                    const readallid = await danzin.chats.all()
                    danzin.setMaxListeners(25)
                    for (let xyz of readallid) {
                        await danzin.chatRead(xyz.jid)
                    }
                    danzin.sendMessage(from, `Pronto`, text, {
                        quoted: {
                            key: {
                                fromMe: false,
                                participant: `0@s.whatsapp.net`,
                ...
                case 'blocklist':
                    ...
                    danzin.sendMessage(from, tekkks.trim(), extendedText, {
                        quoted: mek,
                        contextInfo: {
                ...
                case 'ocr':
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        const com = await danzin.downloadAndSaveMediaMessage(ocrt)
                        reply(ptbr.wait())
                        addFilter(from)
                        await recognize(com, {
                                lang: 'eng+ind',
                ...
                addFilter(from)
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
                        const media = await danzin.downloadAndSaveMediaMessage(encmedia)
                        ran = getRandom('.webp')
                        await ffmpeg(`./${media}`)
                            .input(media)
                            ...
                            .on('end', function() {
                                console.log('Finish')
                                exec(`webpmux -set exif ${addMetadata('DANZIN-BOT', 'danzin Dev')} ${ran} -o ${ran}`, async(error) => {
                                    if (error) return reply(ptbr.stick())
                                    danzin.sendMessage(from, fs.readFileSync(ran), sticker, {
                                        quoted: mek
                                    })
                                    fs.unlinkSync(media)
                                    fs.unlinkSync(ran)
                        ...
                        await ffmpeg(`./${media}`)
                            ...
                            .on('end', function() {
                                ...
                                exec(`webpmux -set exif ${addMetadata('DANZIN-BOT', 'danzin Dev')} ${ran} -o ${ran}`, async(error) => {
                                    if (error) return reply(ptbr.stick())
                                    danzin.sendMessage(from, buff, sticker, {quoted: mek})
                                    fs.unlinkSync(media)
                                    fs.unlinkSync(ran)
                                })
                            })
                ...
                addFilter(from)
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        exec(`ffmpeg -i ${media} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 800:800 ${rano}`, (err) => {
                            fs.unlinkSync(media)
                            exec(`webpmux -set exif ${addMetadata('DANZIN-BOT', 'danzin Dev')} ${rano} -o ${rano}`, async(error) => {
                                buffer = fs.readFileSync(rano)
                                danzin.sendMessage(from, buffer, sticker, {
                                    quoted: mek
                                })
                                fs.unlinkSync(rano)
                            })
                ...
                case 'tts':
                addFilter(from)
                    if (args.length < 1) return danzin.sendMessage(from, `Você deve usar o comando da forma correta:\n${prefix}tts (língua) (texto)\nExemplo: ${prefix}tts pt bom dia\n\nUse: ${prefix}ts para listar todas as línguas`, text, {
                        quoted: mek
                    })
                    const gtts = require('./lib/gtts')(args[0])
                    if (args.length < 2) return danzin.sendMessage(from, 'Cadê o texto?', text, {
                        quoted: mek
                    })
                    dtt = body.slice(8)
                    ranm = getRandom('.mp3')
                    dtt.length > 800 ?
                        ...
                        gtts.save(ranm, dtt, function() {
                            danzin.sendMessage(from, fs.readFileSync(ranm), audio, {
                                quoted: mek,
                                mimetype: 'audio/mp4',
                                ptt: true
                            })
                ...
                case 'bugreport':
                    ...
                    if (args.length >= 300) return danzin.sendMessage(from, 'Máximo 300 caracteres', msgType.text, {
                        quoted: mek
                    })
                    ...
                    danzin.sendMessage('558486067295@s.whatsapp.net', options, text, {
                        quoted: mek
                    })
                ...
                case 'simi':
                    ...
                    try {
                        ...
                        danzin.sendMessage(from, `${anu.success} 🐤`, text, {
                            quoted: mek
                        })
                    } catch {
                ...
                case 'tempo':
                    ...
                    danzin.sendMessage(from, anu, image, {
                        quoted: mek
                    })
                    addFilter(from)
                    break
                ...
                case 'hora':
                    ...
                    //_GIT DO BOT
                case 'git':
                    danzin.sendMessage(from, `https://github.com/Danzinhors/DANZIN-BOT.git`, text, {
                        quoted: mek
                    })
                    addFilter(from)
                    break
                ...
                case 'exteam':
                    danzin.sendMessage(from, `https://api-exteam.herokuapp.com/`, text, {
                        quoted: mek
                    })
                    addFilter(from)
                    break
                ...
                case 'macumba':
                    ...
                    danzin.sendMessage(from, buf, video, {
                        quoted: mek
                    })
                    addFilter(from)
                    break
                ...
                case 'boc':
                    ...
                    if ( /*horaAtual == '08:42' && */ autoHourActivate == "off" && isGroup) {
                        ...
                        for (let _ of groupMembers) {
                            danzin.sendMessage(_.jid, aMimirMp3, MessageType.audio, {
                                mimetype: 'audio/mp4',
                                ptt: true
                            })
                            danzin.sendMessage(_.jid, aMimirSt, sticker)
                        }
                        ammOff = "on"
                ...
                case 'jsonfiles':
                    ...
                    danzin.sendMessage(from, daftarfile, document, {
                        mimetype: 'json',
                ...
                case 'send':
                    ...
                    danzin.sendMessage(nomor + '@s.whatsapp.net', pesan, text)
                    addFilter(from)
                    break
                ...
                case 'attp':
                    ...
                    danzin.sendMessage(from, attp2, sticker, {
                        quoted: mek
                    })
                    break

                case 'pão':
                    if (isOwner) return reply('Oi danzin')
                    else {
                ...
                case 'danzin':
                case 'danzin':
                    await danzin.sendMessage(from, {
                        displayname: "danzin🧙‍♂️",
                        vcard: vcard
                    }, MessageType.contact)
                    addFilter(from)
                    break
                ...
                case 'ownergroup':
                    ...
                    danzin.sendMessage(from, options, text, {
                        quoted: mek,
                        contextInfo: {
                            mentionedJid: [from]
                        }
                ...
                case 'testime':
                    setTimeout(() => {
                        danzin.sendMessage(from, 'O tempo acabou', text)
                    }, 10000)
                    setTimeout(() => {
                        danzin.sendMessage(from, '5 segundos restantes', text)
                    }, 5000)
                    setTimeout(() => {
                        danzin.sendMessage(from, '10 segundos restantes', text)
                    }, 0)
                    addFilter(from)
                    break
                    ...
                    try {
                        axios.get(`https://nekos.life/api/v2/img/meow`).then((res) => {
                            imageToBase64(res.data.url).then((response) => {
                                ...
                                danzin.sendMessage(from, buf, image, {
                                    quoted: mek,
                                    caption: "🐱"
                                })
                            })
                            ...
                            imageToBase64(res.data.url)
                                .then(
                                    (ress) => {
                                        ...
                                        danzin.sendMessage(from, buf, MessageType.image, {
                                            quoted: mek
                                        })
                                    })
                        })
                ...
                case 'togif': // by lindow
                    if ((isMedia && !mek.message.videoMessage || isQuotedSticker) && args.length == 0) {
                        ...
                        const mediaaa = await danzin.downloadAndSaveMediaMessage(encmediaaa)
                        reply(ptbr.wait())
                        addFilter(from)
                        a = await webp2gifFile(mediaaa)
                        mp4 = await getBuffer(a.result)
                        danzin.sendMessage(from, mp4, MessageType.video, {
                            mimetype: 'video/gif',
                            filename: `stick.gif`,
                            quoted: mek,
                            caption: '✅'
                ...
                case 'nulis':
                    ...
                    danzin.sendMessage(from, buff, image, {
                        quoted: mek,
                        caption: '✏📕'
                    })
                    addFilter(from)
                ...
                case 'perfil':
                    ...
                    try {
                        ppimg = await danzin.getProfilePicture(`${sender.split('@')[0]}@c.us`)
                    } catch {
...
`
                    ...
                    danzin.sendMessage(from, its, image, {
                        quoted: mek,
                        caption: pf,
                        contextInfo: {
                            mentionedJid: [sender]
               ...
               //_EFEITO NIGHTCORE PARA AUDIO         
                ...
                addFilter(from)
                    ...
                    media = await danzin.downloadAndSaveMediaMessage(encmmmedia)
                    ran = getRandom('.mp3')
                ...
                case 'slow':
                    ...
                    slo = await danzin.downloadAndSaveMediaMessage(low)
                    ran = getRandom('.mp3')
                ...
                case 'esquilo':
                    ...
                    tup = await danzin.downloadAndSaveMediaMessage(pai)
                    ran = getRandom('.mp3')
                ...
                case 'gemuk':
                    ...
                    gem = await danzin.downloadAndSaveMediaMessage(muk)
                    ran = getRandom('.mp3')
                ...
                addFilter(from)
                    ...
                    media = await danzin.downloadAndSaveMediaMessage(encmediiiaa)
                    ran = getRandom('.mp3')
                ...
                case 'grave':
                    ...
                    bas = await danzin.downloadAndSaveMediaMessage(ass)
                    ran = getRandom('.mp3')
               ...
               /* case 'infogp': 
                ...
                case 'grupinfo':*/
                    ...
                    try {
                        ppUrl = await danzin.getProfilePicture(from)
                    } catch {
  ...
  *◍₊ิ۪͡୭⇾Total de membros: ${participants.length} membros`
                    await danzin.sendMessage(from, buffer, image, {
                        quoted: mek,
                        caption: infogp,
                        contextInfo: {
                ...
                case 'imagem':
                    ...
                    try {
                        ...
                        danzin.sendMessage(from, pok, image, {
                            quoted: mek
                        })
                    } catch {
                ...
                case 'online':
                    ...
                    let online = [...Object.keys(danzin.chats.get(ido).presences), danzin.user.jid]
                    danzin.sendMessage(from, 'Lista de usuários online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, {
                        quoted: mek,
                        contextInfo: {
                            mentionedJid: online
                        }
                ...
                case 'nomeninja':
                    addFilter(from)
                    if (args.length < 1) return reply(`Exemplo: ${prefix}nomeninja danzin`)
                    danzin.updatePresence(from, Presence.composing)
                    nome = body.slice(10)
                    try {
                ...
                case 'setppgc':
                    ...
                    const medipp = await danzin.downloadAndSaveMediaMessage(ftgp)
                    await danzin.updateProfilePicture(from, medipp)
                ...
                case 'ger':
                    ...
                    addFilter(from)
                        ...
                        exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
                            ...
                            danzin.sendMessage(from, nobg, sticker, {
                                quoted: mek
                            })
                            fs.unlinkSync(rano)
                        })
              ...
              // case 'drawing':
                    ...
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        reply(ptbr.waitimg())
                    addFilter(from)
                        owgi = await danzin.downloadAndSaveMediaMessage(ted)
                        tels = body.slice(7)
                        anu = await imgbb("9d7a1bd760e2e3360dbfd40cec4d7ad7", owgi)
              ...
              //  case 'flowertext':
                    ...
                    danzin.sendMessage(from, heh, image, {
                        quoted: mek
                    })
                    break
                    ///_photoOXY
  ...
  *◍₊ิ۪͡୭⇾ Cidade: ${hehe.city}`
                    danzin.sendMessage(from, ccg, text, {
                        quoted: mek
                    })
                    break
   ...
   *◍₊ิ۪͡୭⇾ Validade: ${hehe.result.card.expiration_month}/${hehe.result.card.expiration_year}`
               ...
               // case 'bf4':
                ...
                addFilter(from)
                    ...
                    danzin.updatePresence(from, Presence.composing)
                    if (!isGroup) return reply(ptbr.group())
                    if (args.length < 1) return danzin.reply('escreva algo como aviso')
                    aviso = `Aviso de: @${sender.split("@")[0]}\n\nAviso: ${body.slice(7)}`
                    group = await danzin.groupMetadata(from);
                    ...
                    await danzin.sendMessage(from, options, text)
                    break
                ...
                case 'tagme':
                    ...
                    danzin.sendMessage(from, tagme, text)
                    addFilter(from)
                    break
   ...
   *◍₊ิ۪͡୭⇾ CEP: ${anu.zip_code}`
                    danzin.sendMessage(from, dadosf, text, {
                        quoted: mek
                    })
                    addFilter(from)
                    break
                ...
                case 'tomp3':
                    if (isMedia && !mek.message.imageMessage || isQuotedVideo) {
                        ...
                        addFilter(from)
                        mitri = mek
                        duh = await danzin.downloadAndSaveMediaMessage(mitri)
                        ran = getRandom('.mp3')
                        exec(`ffmpeg -i ${duh} ${ran}`, (err) => {
                            fs.unlinkSync(duh)
                            if (err) return reply('❌falha ao converter video para mp3❌')
                            buffer = fs.readFileSync(ran)
                            danzin.sendMessage(from, buffer, audio, {
                                mimetype: 'audio/mp4',
                                quoted: mek
                            })
                            fs.unlinkSync(ran)
                ...
                case 'block':
                    ...
                    if (!isGroup) return reply(ptbr.group())
                    if (!isOwner) return reply(ptbr.ownerB(ownerName))
                    danzin.blockUser(`${body.slice(8)}@c.us`, "add")
                    danzin.sendMessage(from, `Usuário bloqueado`, text, {
                        quoted: mek
                    })
                    break
                ...
                case 'unblock':
                    if (!isGroup) return reply(ptbr.group())
                    if (!isOwner) return reply(ptbr.ownerB(ownerName))
                    danzin.blockUser(`${body.slice(9)}@c.us`, "remove")
                    danzin.sendMessage(from, `Usuário desbloqueado`, text)
                    break
                ...
                case 'setname':
                    ...
                    danzin.groupUpdateSubject(idgrup, `${body.slice(9)}`)
                    danzin.sendMessage(from, 'nome do grupo alterado', text, {
                        quoted: mek
                    })
                    break
                ...
                case 'setdesk':
                    ...
                    if (!isGroupAdmins) return reply(ptbr.admin())
                    if (!isBotGroupAdmins) return reply(ptbr.Badmin())
                    danzin.groupUpdateDescription(from, `${body.slice(9)}`)
                    danzin.sendMessage(from, 'descrição do grupo alterada', text, {
                        quoted: mek
                    })
                    break
                ...
                case 'setppbot':
                    ...
                    const cuk = await danzin.downloadAndSaveMediaMessage(botpp)
                    await danzin.updateProfilePicture(botNumber, cuk)
                ...
                case 'linkgc':
                    ...
                    if (!isGroupAdmins) return reply(ptbr.admin())
                    if (!isBotGroupAdmins) return reply(ptbr.Badmin())
                    linkgc = await danzin.groupInviteCode(from)
                ...
                case 'toimg':
                    ...
                    medtmg = await danzin.downloadAndSaveMediaMessage(tomg)
                    ran = getRandom('.png')
                ...
                case 'registrar':
                    ...
                    veri = sender
                    try {
                        ppimg = await danzin.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
                    } catch {
...
você se registrou, digite ${prefix}menu para listar meus comandos`
                    ...
                    danzin.sendMessage(from, captionnya, text, {
                        quoted: mek,
                        contextInfo: {
                            mentionedJid: [sender]
                        }
                ...
                case 'cpf1':
                    ...
                    cpf = `${cp1}.${cp2}.${cp3}-${cp4}`
                    danzin.sendMessage(from, `CPF gerado: ${cpf}`, text, {
                        quoted: mek
                    })
                    addFilter(from)
                    break
                ...
                case 'abrirg':
                    ...
                    danzin.groupSettingChange(from, GroupSettingChange.messageSend, false)
                    danzin.sendMessage(from, open, text, {
                        quoted: mek
                    })
                    break
                ...
                case 'clone':
                    ...
                    try {
                        pp = await danzin.getProfilePicture(id)
                        buffer = await getBuffer(pp)
                        danzin.updateProfilePicture(botNumber, buffer)
                ...
                case 'tagimg':
                    ...
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        filePath = await danzin.downloadAndSaveMediaMessage(ncmedia, filename = getRandom())
                        var value = args.join(" ")
                        var group = await danzin.groupMetadata(from)
                        ...
                        danzin.sendMessage(from, ini_buffer, image, options)
                ...
                case 'tagstick':
                    ...
                    if ((isMedia && !mek.message.videoMessage || isQuotedSticker) && args.length == 0) {
                        ...
                        filePath = await danzin.downloadAndSaveMediaMessage(enmedia, filename = getRandom())
                        var value = args.join(" ")
                        var group = await danzin.groupMetadata(from)
                        var member = group['participants']
                        var mem = []
                        ...
                        ini_buffer = fs.readFileSync(filePath)
                        danzin.sendMessage(from, ini_buffer, sticker, options)
                        fs.unlinkSync(filePath)
                    } else {
                ...
                addFilter(from)
                    ...
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        danzin.sendMessage(from, nikhmmp, image, {
                            quoted: mek
                        })
                    } else {
                ...
                addFilter(from)
                    ...
                    mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid[0]
                    try {
                        pp = await danzin.getProfilePicture(mentioned)
                        buffer = await getBuffer(pp)
                ...
                addFilter(from)
                    ...
                    mmedia = await danzin.downloadAndSaveMediaMessage(enncmedia)
                    ran = getRandom('.mp3')
                ...
                addFilter(from)
                    ...
                    mehdia = await danzin.downloadAndSaveMediaMessage(encmmedia)
                    ran = getRandom('.mp3')
            ...
            //   case 'facebookpage':
              ...
              //  case 'laptop':
                    ...
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        danzin.sendMessage(from, dhehe, image, {
                            quoted: mek
                        })
                    } else {
               ...
               // case 'linephoto':
                    ...
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        danzin.sendMessage(from, hehet, image, {
                            quoted: mek
                        })
                    } else {
                        reply('reply imagenya kak!')
              ...
              //  case 'raindrop':
                    ...
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        danzin.sendMessage(from, hehre, image, {
                            quoted: mek
                        })
                    } else {
                        reply('reply imagenya kak!')
              ...
              //  case 'sketch':
                    ...
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        danzin.sendMessage(from, hehae, image, {
                            quoted: mek
                        })
                    } else {
                        reply('reply imagenya kak!')
              ...
              case 'return':
                    if (!isOwner) return reply(ptbr.ownerB(ownerName))
                    arguss = body.slice(8)
                    return danzin.sendMessage(from, JSON.stringify(eval(arguss)), text, {
                        quoted: mek
                    })
                    break
                ...
                case 'exe':
                    ...
                    exec(cmd, (err, stdout) => {
                        if (err) return danzin.sendMessage(from, `EXEC ${err}`, text, {
                            quoted: mek
                        })
                        if (stdout) {
                            danzin.sendMessage(from, stdout, text, term)
                        }
                    })
                    break
                ...
                if (!isOwner) return reply(ptbr.ownerB(ownerName))
                    danzin.updatePresence(from, Presence.composing)
                    danzin.sendMessage('status@broadcast', `${q}`, extendedText)
                    danzin.sendMessage(from, `Texto enviado ao status: ${q}`, text, {
                        quoted: mek
                    })
                    break
                ...
                if (!isOwner) return reply(ptbr.ownerB(ownerName))
                    ...
                    if (isQuotedImage) {
                        ...
                        cihcih = await danzin.downloadMediaMessage(swsw)
                        danzin.sendMessage('status@broadcast', cihcih, image, {
                            caption: `${q}`
                        })
                    }
                    if (args.length < 1) {
                        bur = `Imagem enviada ao status`
                        danzin.sendMessage(from, bur, text, {
                            quoted: mek
                        })
                    } else {
                ...
                if (!isOwner) return reply(ptbr.ownerB(ownerName))
                    ...
                    if (isQuotedVideo) {
                        ...
                        cihcih = await danzin.downloadMediaMessage(swsw)
                        danzin.sendMessage('status@broadcast', cihcih, video, {
                            caption: `${q}`
                        })
                    }
                    if (args.length < 1) {
                ...
                case 'fordward':
                addFilter(from)
                    danzin.sendMessage(from, `${body.slice(10)}`, MessageType.text, {
                        contextInfo: {
                            forwardingScore: 508,
                            isForwarded: true
                        }
                ...
                addFilter(from)
                    ...
                    if (args.length < 1) return
                    saying = teks
                    danzin.sendMessage(from, saying, text)
                    break
                ...
                case 'dado':
                    ...
                    danzin.sendMessage(from, dador, sticker, {
                        quoted: mek
                    })
                    break
                ...
                case 'caracoroa':
                    ...
                    danzin.sendMessage(from, cararoa, sticker, {
                        quoted: mek
                    })
                    break
                ...
                case 'sn':
                    ...
                    gosto = body.slice(4)
                    addFilter(from)
                    if (args.length < 1) return danzin.sendMessage(from, `Você deve fazer uma pergunta...\nExemplo: ${prefix}sn O danzin é um baiano preguiçoso?`, text, {
                        quoted: mek
                    })
                ...
                case "ppt":
                    ...
                    if (vit == "derrota") {
                        var tes = "A vitória é do DANZIN-BOT"
                    }
                    if (vit == "empate") {
                        var tes = "O jogo terminou em empate"
                    }
                    reply(`DANZIN-BOT jogou: ${pptb}\nO jogador jogou: ${args}\n\n${tes}`)
                ...
                addFilter(from)
                    ...
                    danzin.sendMessage(from, casal, text, {
                        quoted: mek,
                        contextInfo: {
                            "mentionedJid": [ag]
                        }
...
${vitr}`
                    ...
                    danzin.sendMessage(from, slott, text, {
                        quoted: mek
                    })
                    break
                ...
                addFilter(from)
                    ...
                    if (args.length < 1) return danzin.sendMessage(from, `Você precisa digitar da forma correta\nExemplo: ${prefix}chance do danzin ser um trouxa`, text, {
                        quoted: mek
                    })
                    random = `${Math.floor(Math.random() * 100)}`
                    hasil = `A chance ${body.slice(8)}\n\né de... ${random}%`
                    danzin.sendMessage(from, hasil, text, {
                        quoted: mek,
                        contextInfo: {
                            mentionedJid: [sender]
                        }
                ...
                addFilter(from)
                    ...
                    if (morte == "Tinha uma bala no tambor, POW!") {
                        setTimeout(() => {
                            danzin.sendMessage(from, figb, sticker, {
                                quoted: mek
                            })
                        }, 2100)
                    }
                    setTimeout(() => {
                        danzin.sendMessage(from, morte, text, {
                            quoted: mek
                        })
                        danzin.groupRemove(sender)
                    }, 2300)
                    break
                ...
                case 'ttt':
                    ...
                    if (!isGroup) {
                        ...
                        danzin.sendMessage(from, `🌀1️⃣2️⃣3️⃣\n🅰️${esp.a1}${esp.a2}${esp.a3}\n🅱️${esp.b1}${esp.b2}${esp.b3}\n©️${esp.c1}${esp.c2}${esp.c3}`, text)
                        danzin.sendMessage(from, `Caso não saiba como jogar digite: ${prefix}ttthelp`, text)
                        setTimeout(() => {
                ...
                case 'ttthelp':
                    danzin.sendMessage(from, ttthelp(prefix), text)
                    break
                ...
                case 'tttme':
                    ...
                    danzin.sendMessage(from, tttme(pushname, getTTTwins(sender), getTTTdefeats(sender), getTTTties(sender), getTTTpoints(sender)), text, {
                        quoted: mek
                    })
                    break
                ...
                case 'tttrank':
                    ...
                    try {
                        ...
                        console.log(err)
                        await danzin.sendMessage(from, `Humm, é necessário que no mínimo 3 pessoas tenham jogado...`, text, {
                            quoted: mek
                        })
                    }
                    break
                        ...
                        if (coordX != 'a1' && coordX != 'a2' && coordX != 'a3' &&
                            ...
                            if (WinnerX()) {
                                if (isLevelingOn) {
                                    ...
                                    danzin.sendMessage(from, `🎉🎉 VITÓRIA DO JOGADOR 🎉🎉\n\n➣  RECOMPENSA: +${randomTTTXP} XP 🔮`, text)
                                } else {
                                    danzin.sendMessage(from, `🎉🎉 VITÓRIA DO JOGADOR 🎉🎉`, text)
                                }
                                ...
                                if (isLevelingOn) {
                                    ...
                                    danzin.sendMessage(from, `🎉🎉 VITÓRIA DO DANZIN-BOT 🎉🎉\n\n➣  PUNIÇÃO: ${randomTTTXP} XP 🔮`, text)
                                } else {
                                    danzin.sendMessage(from, `🎉🎉 VITÓRIA DO DANZIN-BOT 🎉🎉`, text)
                                }
                                ...
                                if (isLevelingOn) {
                                    danzin.sendMessage(from, `🎉🎉 EMPATE 🎉🎉\n\n➣  NÃO HÁ GANHOS NEM PERDAS`, text)
                                } else {
                                    danzin.sendMessage(from, `🎉🎉 EMPATE 🎉🎉`, text)
                                }
...
　 　　。　　 　　　　ﾟ　　　.　      　　　.
,　　　　.                  .`
                        //  danzin.groupRemove(from, mentioned)
                        ...
                        axios.get('https://nekos.life/api/v2/img/nsfw_neko_gif').then(res => {
                            imageToBase64(res.data.url)
                                .then(
                                    (ress) => {
                                        var buf = Buffer.from(ress, 'base64')
                                        danzin.sendMessage(from, buf, MessageType.video, {
                                            mimetype: 'video/gif',
                                            filename: `stick.gif`,
                                            quoted: mek,
                                            caption: '✅'
                ...
                default:
                    ...
                    if (isCmd) {
                        if (!mek.key.fromMe) {
                            ...
                            danzin.sendMessage(from, hsl, text, {
                                quoted: mek,
                                contextInfo: {
                                    mentionedJid: [sender]
                                }
                            })
                           const baino = `558486067295@s.whatsapp.net`
                          const tamme = {
                    ...
                    danzin.sendMessage(from, tamme, text)
                        }
                    }
                 }
                        ...
                        if ((isGroup && isMedia & isAuto && !mek.message.imageMessage || isQuotedVideo)) {
                            ...
                            await ffmpeg(`./${media}`)
                                ...
                                .on('end', function() {
                                    console.log('Finish')
                                    buff = fs.readFileSync(ran)
                                    danzin.sendMessage(from, buff, sticker)
                                })
                                .addOutputOptions([`-vcodec`, `libwebp`, `-vf`, `crop=w='min(min(iw\,ih)\,650)':h='min(min(iw\,ih)\,650)',scale=320:320,setsar=1,fps=15`, `-loop`, `0`, `-ss`, `00:00:00.0`, `-t`, `00:00:10.0`, `-preset`, `default`, `-an`, `-vsync`, `0`, `-s`, `512:512`])
                                .toFormat('webp')
                                .save(ran)
